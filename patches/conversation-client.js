window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		/** One browser-only draft descriptor; only its id enters input state. */
		...
		/** Strict image classification: raster MIME AND a raster extension (or a
		 *  nameless clipboard bitmap). Weird-MIME files (.pkg, .dmg, …) never
		 *  become image parts. */
		function isImageFile(file) {
			if (file.type !== "image/png" && file.type !== "image/jpeg" && file.type !== "image/webp" && file.type !== "image/gif") return false;
		...
		var ConversationController = class extends _deepseek_ai_cordis.Service {
			...
			*/
			async sendSession(session, text, imageIds, mode) {
				...
				const content = [...await this.serializeImages(attachments.map((attachment) => attachment.file), sessionId), ...text === "" ? [] : [{
					type: "text",
					text
				}]];
			...
			*/
			...
			/** Convert browser files to canonical base64 prompt parts (images), durable file blocks (files/folders), or text parts. */
			serializeImages(images, sessionId = "") {
				return Promise.all(images.map(async (file) => {
					// Folder upload placeholder: the desktop bridge already copied the
					// whole folder into the session directory; describe it, don't save again.
					if (typeof file.__dshFolderPath === "string" && file.__dshFolderPath !== "") {
						const folderName = file.name === "" ? "folder" : file.name;
						const stats = file.__dshFolderStats ?? {};
						const shortPath = typeof file.__dshFolderShortPath === "string" && file.__dshFolderShortPath !== "" ? file.__dshFolderShortPath : file.__dshFolderPath;
						return [
							{ type: "file", fileKind: "folder", name: folderName, size: stats.totalBytes ?? 0, files: stats.files ?? 0, path: file.__dshFolderPath, shortPath },
							{ type: "text", text: `[The user attached the folder "${folderName}" (${stats.files ?? 0} files, ${formatByteSize(stats.totalBytes ?? 0)}). The whole folder has been copied to disk at "${file.__dshFolderPath}" — read files inside it with your tools (list with ls/glob, read text files, extract binaries) when the task needs them. This is NOT an image attachment.]` }
						];
					}
					if (isImageFile(file)) {
						return [{
					...
					const displayName = relPath !== "" ? relPath : name;
					const sizeText = file.size < 1024 ? `${file.size} B` : file.size < 1048576 ? `${(file.size / 1024).toFixed(1)} KB` : `${(file.size / 1048576).toFixed(2)} MB`;
					// Save non-inlineable files into the session's own folder (deleted with the session).
					const saveToDisk = async () => {
						if (typeof window === "undefined" || typeof window.__DSH_SAVE_UPLOAD__ !== "function") return null;
						try {
							const saved = await window.__DSH_SAVE_UPLOAD__(file, sessionId);
							return saved !== null && typeof saved === "object" && typeof saved.path === "string" && saved.path.length > 0 ? saved : null;
						} catch {
							return null;
						}
					};
					if (isTextLike(file) && file.size <= 524288) {
						const text = await file.text();
						// Also persist so the card can show the real file icon (and
						// the agent can read it from disk later); content stays inline.
						const saved = await saveToDisk();
						const savedPath = saved?.path ?? null;
						return [
							{ type: "file", fileKind: "file", name: displayName, size: file.size, mediaType: file.type || "unknown", ...savedPath !== null ? { path: savedPath, shortPath: saved.shortPath } : {} },
							{ type: "text", text: `[The user attached the file "${displayName}" (${sizeText}, ${file.type || "unknown type"}). Its full text content follows, between <attached-file> markers.${savedPath !== null ? ` It is also saved on disk at "${savedPath}".` : ""}]\n<attached-file name="${displayName}">\n${text}\n</attached-file>` }
						];
					}
					if (isTextLike(file)) {
						const saved = await saveToDisk();
						const savedPath = saved?.path ?? null;
						const readNote = savedPath !== null
							? ` It has been saved to disk at "${savedPath}" — read it with your tools when the task needs its content.`
							: " It is too large to inline fully; ask the user where it is on disk if you need to read it with a tool.";
						return [
							{ type: "file", fileKind: "file", name: displayName, size: file.size, mediaType: file.type || "unknown", ...savedPath !== null ? { path: savedPath, shortPath: saved.shortPath } : {} },
							{ type: "text", text: `[The user attached the text file "${displayName}" (${sizeText}, ${file.type || "unknown type"}).${readNote}]` }
						];
					}
					// Binary attachment: persist to disk so the agent's tools can read it.
					const saved = await saveToDisk();
					const savedPath = saved?.path ?? null;
					const readNote = savedPath !== null
						? ` It has been saved to disk at "${savedPath}" — read it with your tools (e.g. pdftotext, textutil, unzip -l) when the task needs its content. This is NOT an image attachment — do not use the vision/看图 tool on it.`
						: " Its binary content cannot be inlined into the chat; ask the user where it is on disk if you need to read it with a tool.";
					return [
						{ type: "file", fileKind: "file", name: displayName, size: file.size, mediaType: file.type || "unknown", ...savedPath !== null ? { path: savedPath, shortPath: saved.shortPath } : {} },
						{ type: "text", text: `[The user attached the binary file "${displayName}" (${sizeText}, ${file.type || "unknown type"}).${readNote}]` }
					];
		...
		/** Heuristic: can this browser file be inlined into a prompt as text? */
		/** Human byte size for folder stats captions. */
		...
		function isTextLike(file) {
			...
			if (file.type === "application/json" || file.type === "application/xml" || file.type === "application/yaml" || file.type === "application/x-yaml" || file.type === "application/javascript" || file.type === "application/x-javascript" || file.type === "application/typescript" || file.type === "application/x-typescript") return true;
			const name = file.name.toLowerCase();
			const codeExts = [".js", ".mjs", ".cjs", ".jsx", ".ts", ".tsx", ".py", ".rb", ".go", ".rs", ".java", ".c", ".h", ".cpp", ".hpp", ".cs", ".php", ".sh", ".bash", ".zsh", ".fish", ".sql", ".vue", ".svelte", ".scala", ".kt", ".swift", ".pl", ".lua", ".r", ".dart", ".toml", ".ini", ".cfg", ".conf", ".env", ".gradle", ".properties"];
		...
		* (running/removed/promptError) are self-selected via useSession. */
		...
		function InputBar({ useSession, useInput, inputActions, keyboard, addImages, removeImage, draftImages, resolveSubmitMode, toggleCommandMenu, stop, command, t, renderSlot, useNotices, useLexicon, useMenuLauncher, useProjection, sessionId, variant, disabled: inert = false, blocked, workspacePickerOpen = false, onRequestWorkspace, placeholder, accessory, overlay, leftItems, rightItems, footer }) {
			...
			// Global file-intake bridge for desktop "+" menu and other DOM-level triggers.
			(0, react.useEffect)(() => {
				window.__DSH_ADD_FILES__ = (files) => {
					intakeImages(files);
				};
				return () => {
					if (window.__DSH_ADD_FILES__) window.__DSH_ADD_FILES__ = void 0;
				};
			});
			(0, react.useEffect)(() => {
			...
			const intakeImages = (0, react.useCallback)((files) => {
				...
				const otherFiles = files.filter((file) => imageFiles.indexOf(file) === -1);
				const imageAttachments = attachments.filter((attachment) => attachment.isImage === true);
				const rejected = (() => {
					if (imageLimits !== void 0 && imageFiles.length > 0) {
						...
						if (imageAttachments.length + imageFiles.length > imageLimits.maxImagesPerMessage) return t("image.tooMany", { count: imageLimits.maxImagesPerMessage });
						...
						if (imageAttachments.reduce((sum, attachment) => sum + attachment.file.size, 0) + imageFiles.reduce((sum, file) => sum + file.size, 0) > imageLimits.maxMessageImageBytes) return t("image.totalTooLarge", { size: imageSizeText(imageLimits.maxMessageImageBytes) });
					}
					const rejectedOther = otherFiles.length > 0 ? addImages(otherFiles) : null;
					if (rejectedOther !== null) return rejectedOther;
					if (imageFiles.length > 0) return addImages(imageFiles);
					return null;
				})();
									...
									children: [
										(0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Tooltip, {
											...
											children: (0, react_jsx_runtime.jsx)("button", {
												...
												onClick: () => {
													// Desktop shell owns a dedicated upload-only menu;
													// the plain command menu stays reachable via "/".
													if (typeof window !== "undefined" && typeof window.__DSH_OPEN_UPLOAD_MENU__ === "function") {
														window.__DSH_OPEN_UPLOAD_MENU__();
														return;
													}
													onToggleCommandMenu();
												},
		...
		*/
		...
		function contentParts(content) {
			...
			for (const block of content) {
				const b = block;
				if (b.type === "file" && typeof b.name === "string") {
					files.push(normalizeFileBlock(b));
		...
		/** Normalize one durable file block into the chip's display shape. */
		function normalizeFileBlock(block) {
			const sizeBytes = typeof block.size === "number" ? block.size : 0;
			const folderFiles = typeof block.files === "number" ? block.files : 0;
			return {
				kind: block.fileKind === "folder" ? "folder" : "file",
				name: block.name,
				size: block.fileKind === "folder"
					? `${folderFiles} 个文件 · ${formatByteSize(sizeBytes)}`
					: formatByteSize(sizeBytes),
				path: typeof block.shortPath === "string" && block.shortPath !== "" ? block.shortPath : (typeof block.path === "string" ? block.path : "")
			};
		}
		/** Parse legacy `📎 附件：…` / `📁 文件夹：…` captions from older messages. */
		...
		/** Extension → emoji fallback for files without a disk path (inlined text). */
		...
		/** File/folder attachment card rendered above the bubble, image-gallery style. */
		...
		*/
		...
		const UserMessageNodeView = (0, react.memo)(function UserMessageNodeView({ node, loadImage, t }) {
			...
			const extraEdit = showEdit ? (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Tooltip, {
				label: "编辑",
				side: "bottom",
				children: (0, react_jsx_runtime.jsx)("button", {
					type: "button",
		...
		* has not exposed a visible box yet. */
		...
		function promptTargetKey(order, nodes, target) {
			...
			for (const key of order) {
				...
				if (messageId === "" && text !== "" && contentParts(data.content).text.trim() === text) {
					fallback.push({ key, distance: createdAt > 0 ? Math.abs((Number(data.time) || 0) - createdAt) : 0 });
				}
			}
