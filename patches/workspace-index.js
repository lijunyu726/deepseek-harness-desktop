...
var WorkspaceRegistry = class extends Service {
	...
	*
	* The 0.1.5 line dropped this from the registry while keeping
	* `archiveSession`, so the desktop shell's 归档管理 page restores it here:
	* the archive set is the only state involved, so clearing the id is the
	* whole operation — workspace accounting keeps its slot either way.
	...
	*/
	...
	/**
	* Permanently purge one session: workspace accounting, the archive set,
	* the header index, and the durable log file.
	*
	* The 0.1.5 line dropped this from the registry and deleted the ApiProxy
	* that used to own the teardown, so the desktop shell owns the whole
	* sequence: callers dispose the live agent first, then call this to purge
	* everything durable. A missing log file or an already-forgotten session
	* resolves rather than throwing — permanent deletion is idempotent, which
	* is what a batch delete from the 归档管理 page needs.
		...
		return this.enqueueOperation(async () => {
			...
			if (typeof path === "string" && path.length > 0) {
				try {
					...
					/* the log is already gone — deletion stays idempotent */
				}
			}
