...
*
* The 0.1.5 attachment store no longer exposes a `root` property, so the path
* comes from the seam's `imageHostPath()` instead of being rebuilt here.
*/
function desktopVisionMcpContent(ctx, content) {
	return content.flatMap((block) => {
		if (block.type !== "image") return [block];
		const attachment = block.attachment;
		const objectPath = ctx.attachments.imageHostPath(attachment);
		if (typeof objectPath !== "string" || objectPath.length === 0) throw new AttachmentError("GUI image delegation requires the local Harness attachment store.", "VISION_MCP_LOCAL_STORE_REQUIRED");
