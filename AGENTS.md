...
- **删除会话只有归档管理一个入口**：工作区/侧边栏会话行不提供「删除会话」（官方 0.1.5 的 workspace 行菜单只有重命名/复制/归档；v1.4.2 的行内删除来自已废弃的 workspace-client 覆盖，已砍掉）。永久删除统一在「设置 → 归档管理」：`deleteSessions` remote → 逐会话停活体 agent（`live.cancel` + `whenIdle` + `scope.dispose`，每步有超时兜底）→ `workspaceRegistry.deleteSession(id)`（解绑记账 → 移出归档集 → 删日志 → 清索引，幂等）。**0.1.5 删除了 ApiProxy 与 registry 的 `deleteSession` / `unarchiveSession`，这两个方法由 `patches/workspace-index.js` 补回**；不得改回依赖 ApiProxy。支持单选（行内二次确认）与多选批量（顶部批量条确认）。不得为恢复行内删除重新引入 workspace-client 覆盖。
