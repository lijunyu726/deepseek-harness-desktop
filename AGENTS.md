...
- 改动经「验证方式」确认后，先递增根 `package.json` 版本号，再 `npm run dist` 在 `release/` 打出新版本 DMG+zip；`audit:release` 未通过的产物不得交付。版本号必须高于 Git 历史和 Releases 中出现过的全部正式版本，严禁回退；当前发布版本是 `1.4.8`。
...
- **侧边栏运行徽标**：`JobsBadgeController`（挂 `conversation.input.right`、return null）用 `useSessions` 订阅全局 `jobsBySession`（与官方 JobListAction 同源），把「N 运行中」徽标插进侧边栏会话行——覆盖官方状态点缺失的「后台任务」维度（如 strix 在跑而回合不活跃的会话）。行定位依赖官方 workspace 行类名（`YDXeBa_sessionRow/Title/time`，构建产物 hash），升级官方包后必须同步核对；行标题 ↔ store `byId` 标题匹配（displayTitle 优先）。数据是 live 帧驱动的：任务启动时页面需处于连接状态（mux 已建立）才有帧；与官方 JobListAction 行为一致，不做额外兜底。
