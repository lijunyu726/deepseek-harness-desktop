...
- 输入区点模型芯片弹出鲸鱼变阻器：DeepSeek 模型固定六档 `Flash·Off → Flash·High → Flash·Max → V4 Pro·Off → V4 Pro·High → V4 Pro·Max`，每档一套 24 帧鲸鱼动画，档位越高游动越快；
- **鲸鱼就是滑块**：按住鲸鱼在轨道里拖动，或点轨道空白处直接跳档；蓝系渐变填充、星尘粒子、档位圆点刻度（走过的点亮、未走过的暗）；
- 换成其他商家模型时按该模型的实际思考档位自动生成（几档就几档），无档位模型只显示芯片并提示不支持调节；
- 「高级 >」入口提供 Codex 式两行菜单（模型 / 思考强度），可切换任意厂商模型与推理档位。

### 💰 余额徽章一键跳转充值

- 会话页右上角余额徽章可直接点击，一键跳转 DeepSeek 开放平台充值页（platform.deepseek.com/top_up），刷新按钮不受影响。

### 📊 用量面板

- 设置 → 用量：余额摘要 + 近期会话 token 用量 + **近一年每日用量热力图**（365 天）；
- 打开时先渲染本地缓存，再由后台子进程增量扫描会话日志静默更新，不用等 zstd 解码；扫描进程崩溃不影响服务。

### ⏰ 高峰 / 非高峰时段提示

- 输入卡工具行中央常驻读数，仅两个标签：琥珀点「高峰时段」/ 绿点「非高峰时段」；
- 高峰时段为北京时间 9:00–12:00 与 14:00–18:00，悬停显示完整计费规则（非高峰价为高峰价的一半）与当前北京时间，每 30 秒自动刷新；
- 空白新会话同样显示；手机窄屏下改为工具行行内左侧常驻。

### 📱 移动端访问（手机局域网访问）

- 设置 → 应用 → 「移动端访问」开关：开启后服务监听局域网并在设置页内直接显示局域网地址 + 二维码；
- 手机连同一 Wi-Fi 打开地址（或扫码）即用同一服务：App 式窄屏布局（侧边栏抽屉 + 汉堡按钮 + 全屏会话）、断线自动补拉任务进度；与桌面端同一份配置、同一套会话，没有同步层。

### 📎 任意文件 / 文件夹上传

- 「+」菜单、粘贴（Cmd+V）、拖放三条路径都能上传任意类型文件；文本（txt/md/代码 ≤512KB）内容直接内联给模型；
- 二进制与大文本保存到当前会话目录 `~/.dsh/sessions/<工作区>/<会话ID>/uploads/`，Agent 用工具读取，删除会话时一并递归清理（归档保留）；
- 文件夹走原生目录选择器整体递归上传（上限 2000 文件 / 200MB / 深度 8），消息里是一个 📁 文件夹附件；
- 文件卡片显示真实 macOS 系统图标；图片分类双校验，pkg/dmg 等不会误入图片通道。

### 🖼️ 图片发送与视觉桥

- 会话里以图片本体显示（桌面与手机端一致），多图全部保留；描述文字只发给模型、不污染聊天记录；
- 纯文本模型自动走 vision MCP 视觉桥：图片保存为本地附件，模型被引导调用 `mcp__vision__describe_image` 读取，不绕过该模型的计费与额度。

### @会话 提及

- 输入 `@` 弹出触发菜单（按标题 / 会话 id / 工作目录过滤），选中插入以会话标题为标签的 chip；
- 发送时把被引用会话的当前上下文快照作为只读 recall 上下文注入本轮，实现跨会话引用。

### 🕐 历史 Prompt 时间轴

- 会话页左缘 Codex 同构时间轴：细横线 + 鼠标高斯鱼眼，悬停显示 Prompt 正文，点击把该条 Prompt 填回输入框；
- 按会话隔离、最多 100 条、相同文本自动去重；桌面与手机共享。

### ✏️ 消息原位编辑

- ESC 停止生成后点最后一条消息的铅笔，消息原位变成编辑器（取消 / 发送）。

### 🗑️ 会话 / 工作区 / 归档管理

- 会话行菜单删除会话：二次确认、停掉活跃 Agent、永久删除持久日志；
- 删除工作区连带永久删除其中全部会话（弹窗明确警告，文件夹本身保留）；
- 归档管理页单个 + 多选批量彻底删除，删除后侧边栏立即刷新、不会残留在「未分组」。

### ⚙️ 全局约束规则

- 设置 → 全局约束规则直接读写注入每个会话的 `~/.dsh/AGENTS.md`（对应 Codex 的自定义指令），打开即载入现有内容，保存后新会话生效。

### 🚀 启动与运行体验

- Codex 风格启动动画（终端网格 + 鲸鱼 logo + 逐字打字 + 状态轮播），服务就绪淡出切换，失败自动切红色错误态；
- 菜单栏托盘、单实例 + 僵尸锁自愈；桌面壳被强杀时服务子进程随之一并退出，不遗留孤儿进程；
- 应用本体可随意移动：首次启动自动重建插件软链、改写看图工具命令、重拍局域网可信名单；
- Persistent Bash 快速结算修复（官方 rc.7 修复回植 rc.6）：命令不再退化为数秒静默等待；
- 无需系统安装 Node（Electron 内置 Node 22.22.1）；数据沿用 `~/.dsh`，与终端配置互通。
...
- 会话页左缘（抽屉右侧）提供与 Codex 同构的「历史 Prompt 时间轴」：一条 Prompt 对应一根左端对齐的 6×1px 细横线，8px 节距，时间从上向下推进（最新在最下）。鼠标移动时，邻近横线只沿水平方向按像素距离形成高斯鱼眼，最接近光标的横线约伸长至 26px，线条始终保持 1px 粗；右侧气泡不显示标题、日期、序号、箭头或描边，只显示最多四行 Prompt 正文并与当前横线垂直居中。点击横线通过输入状态机把该条 Prompt 填回输入框。宿主只在根 agent 的 `agent/pre-step` 接受边界记录真实用户 Prompt，最多保留 100 条、相同文本自动去重；历史保存在 `$DSH_HOME/desktop/prompt-history.json`（权限 600），桌面和手机端共享，不进入仓库或安装包。v1.3.2 起每条记录带会话归属，时间轴**只显示当前会话**的 Prompt，不再混入其他会话的历史。
