...
- 高峰/非高峰时段提示（v1.3.1，v1.3.2 移到顶部栏中央，v1.3.3 常驻所有会话）：会话页**顶部栏（标题行）中央**常驻读数，仅两个标签——琥珀点「高峰时段」/ 绿点「非高峰时段」。北京时间 9:00–12:00 与 14:00–18:00 为高峰；悬停显示完整计费规则（非高峰价为高峰价的一半）与当前北京时间，每 30 秒自动刷新。挂载在 `conversation.input.dock`（新会话 blank 阶段也渲染），组件测量会话面板矩形后固定定位在标题行位置，不依赖 header 是否可见。
