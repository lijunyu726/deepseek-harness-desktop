...
2、侧边栏运行徽标：有后台任务（如 strix）正在运行的会话，在侧边栏会话行显示「N 运行中」徽标（蓝点 + 图标），避免把"有 agent 在后台干活"的会话误判为空闲；任务结束自动消失。

3、余额跳转：右上角余额徽章可点击，每完成一次任务刷新，直达 DeepSeek 开放平台充值页。

4、用量面板：余额摘要、近期 token 用量、近一年每日热力图；先读缓存、后台子进程增量扫描，卡不住页面。

5、高峰/非高峰提示：工具行中央常驻，琥珀点高峰（北京时间 9:00–12:00、14:00–18:00）、绿点非高峰。

6、移动端访问：开关一开就能局域网访问，设置页直接出地址和二维码，手机打开自动切 App 式布局，和桌面同一份配置和会话。

7、文件与图片：任意文件/文件夹上传，存会话目录、删会话一并清掉；卡片显示真实 macOS 图标；官方 Vision 模型原生接收图片并优先复用 DeepSeek Files API，失败时自动回退内联图片。

8、其他：@ 提及其他会话获取上下文、历史 Prompt 时间轴、ESC 后原位编辑重发、会话/工作区/归档批量彻底删除、设置里直接编辑全局约束规则；启动动画、托盘、单实例、路径自愈都在；回植了 persistent Bash 修复，命令不再白等三秒半。
