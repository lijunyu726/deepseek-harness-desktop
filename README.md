...
> 本分支是 **2026-08-17 迁移前的旧版代码**，仅作存档保留，不再维护。
> 当前活跃版本请切换 `main` 分支（项目已迁至 `/Volumes/S690/codes/deepseek-harness-desktop`）。
> 本目录保留完整 node_modules 与历史安装包（release/），只上传了源代码。
...
- 服务子进程预加载 `main/child-guard.mjs` 监视所属 Electron 进程；桌面壳崩溃或被强杀后，服务会自行退出，避免遗留多套 `dsh web` 与 MCP 进程。用量页的按日热力图会扫描原始会话日志（`assistant/message` 帧按本地日期分桶）：Electron 内置 Node 的 zstd 原生解码存在随机 SIGTRAP（同步/异步/流式路径均复现过），因此所有解压都隔离在独立的扫描子进程 `assets/usage-scan.mjs` 中执行（崩溃不影响服务进程），并按文件 mtime+size 缓存，任务完成、窗口聚焦与每分钟定时刷新时只重扫发生变化的日志。
- 开发调试时如需与已安装实例共存，用 `DSH_USER_DATA_DIR=/tmp/dsh-uitest` 隔离开发实例的配置目录（锁、缓存、日志互不干扰）。
- 修改 `packages/dsh-desktop-instructions/` 后：`npm pack` 到 tarball 再 `npm install` tarball 重装，然后 `npm run dist` 重新打包。

## 打包

```bash
npm run dist          # 产出 release/DeepSeek Harness-1.2.18-arm64.dmg 与 .zip
