...
- 会话页左缘（抽屉右侧，Codex 式）新增「历史 Prompt 时间轴」：无外壳的细短刻度（一条 Prompt 一根，最新在最上），不压抽屉、不贴聊天框；宿主只在根 agent 的 `agent/pre-step` 接受边界记录真实用户 Prompt，最多保留 100 条、相同文本自动去重。鼠标悬停时整列刻度以悬停那根为中心向外逐级变小（鱼眼扩散），并在右侧弹出带箭头的预览气泡（标题 + 时间 + 原文），点击刻度即把该条 Prompt 填回输入框。轨道层级低于设置等浮层，打开设置面板时不会压在上面。历史保存在 `$DSH_HOME/desktop/prompt-history.json`（权限 600），桌面和手机端共享，不进入仓库或安装包。
