...
- 输入区的模型选择器默认保持系统原生样式（DeepSeek Honeycomb 风格芯片，只显示模型与推理档位）；**点击芯片**才弹出鲸鱼思考强度变阻器，固定六档顺序为 `Flash·Off → Flash·High → Flash·Max → V4 Pro·Off → V4 Pro·High → V4 Pro·Max`。六档各使用一套透明底 24 帧动画：Flash 保持轻快，Pro 的动作和表情更有力量，因而 Flash 最高档不会显得比 Pro 更努力。**鲸鱼就是滑块拇指**：游在轨道里、位置就是当前档位，按住鲸鱼拖动即可切换档位（点击轨道空白处也可直接跳档）；档位越高浮游/摆尾越快。轨道填充为蓝系渐变（无紫色），满档时填充到头；填充内有随档位增多的星尘粒子和流光扫过；六个档位圆点刻度排在轨道内部底部一行（走过的点亮、未走过的暗，鲸鱼浮在上方不遮挡）。点击外部或 Esc 收起。动画遵循系统“减少动态效果”设置；桌面窗口与手机端使用同一组本地同源素材。
...
- 会话页左缘（抽屉右侧，Codex 式）新增「历史 Prompt 时间轴」：无外壳的细短刻度（一条 Prompt 一根，最新在最上），不压抽屉、不贴聊天框；每根刻度虽细（可视 10×2.5px），但带 14×12.5px 的隐形点击区，很容易点中。宿主只在根 agent 的 `agent/pre-step` 接受边界记录真实用户 Prompt，最多保留 100 条、相同文本自动去重。鼠标在刻度列上移动时，整列刻度按与光标的像素距离做**连续鱼眼扩散**（峰值 1.5 倍、向两侧对称衰减成圆顶、下限为默认长度——不会比平时更短），像 Dock 放大一样丝滑跟随；右侧弹出带箭头的预览气泡（标题 + 时间 + 原文，跟随最近的刻度），点击刻度即把该条 Prompt 填回输入框。轨道层级低于设置等浮层，打开设置面板时不会压在上面。历史保存在 `$DSH_HOME/desktop/prompt-history.json`（权限 600），桌面和手机端共享，不进入仓库或安装包。
