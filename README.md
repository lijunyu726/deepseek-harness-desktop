...
- 高峰/空闲时段提示（v1.3.1）：会话页输入框下方常驻环境读数。北京时间 9:00–12:00 与 14:00–18:00 为高峰时段（琥珀点「高峰时段 · 标准价 · BJ HH:mm」），其余为空闲时段（绿点「空闲时段 · 半价 · BJ HH:mm」）；悬停显示完整计费规则（空闲价为高峰价的一半）与当前北京时间，每 30 秒自动刷新。北京时间无夏令时，纯客户端按 UTC+8 换算，无需网络。
