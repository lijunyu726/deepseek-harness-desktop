...
- 会话页左缘（抽屉右侧，Codex 式）新增「历史 Prompt 时间轴」：无外壳的细短刻度（一条 Prompt 一根，最新在最上），不压抽屉、不贴聊天框；宿主只在根 agent 的 `agent/pre-step` 接受边界记录真实用户 Prompt，最多保留 100 条、相同文本自动去重。鼠标在刻度列上移动时，整列刻度按与光标的像素距离做**连续鱼眼扩散**（最近 1.6 倍、越远越小、下限为默认长度——不会比平时更短），像 Dock 放大一样丝滑跟随；右侧弹出带箭头的预览气泡（标题 + 时间 + 原文，跟随最近的刻度），点击刻度即把该条 Prompt 填回输入框。轨道层级低于设置等浮层，打开设置面板时不会压在上面。历史保存在 `$DSH_HOME/desktop/prompt-history.json`（权限 600），桌面和手机端共享，不进入仓库或安装包。
